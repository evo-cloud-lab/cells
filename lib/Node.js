    ...
    _invoke: function (intermediateState, action, autoTransitState) {
        process.nextTick(function () {
            ...
            if (typeof(fn) == 'function') {
                process.nextTick(function () { fn.call(this, {}); }.bind(this.node._interior));
...
var Node = Class(ManagementObject, {
    ...
    setup: function (callback) {
        flow.steps()
            ...
            .next(function (next) {
                plugins.connect(this, 'cells:node.interior', {
                    name: this.properties.interior || (this._image.manifest.node && this._image.manifest.node.interior),
                    data: this.pluginData(this.properties, this.properties.interior, _.extend({
                                monitor: this._monitorEvent.bind(this)
                            }, this._image.manifest.node || {})),
                    required: true
                }, function (err, interior) {
                    !err && (this._interior = interior);
                    interior && (this._interiorState = 'offline');
                    next(err);
                }.bind(this));
            })
            .with(this)
    ...
    dump: function () {
        return {
            ...
            interior: {
                name:  this.properties.interior,
                state: this.interiorState
            },
    ...
    _transitionError: function (err) {
        this.logger.error('TRANSITION ERROR: exp=%s cur=%s valid=%j', err.expectation, err.actual, err.accepts);
        // clear expectation
        this._transits.setExpectation();
    },
