...
var Node = Class(ManagementObject, {
    ...
    dump: function () {
        return {
            ...
            name: this.name,
            nics: this._interior.nics,
            state: {
                current: this.state,
                expected: this.expectedState,
            },
            status: this.recentStatus,
            interior: _.extend({ name: this._interior.name },
                               typeof(this._interior.dump) == 'function' ? this._interior.dump() : {}),
    ...
    _monitorEvent: function (event, data) {
        this.logger.debug('[MONITOR] %s: %j', event, data);
        switch (event) {
            case 'error':
                this.logger.logError(data, {
                    message: '[INTERIOR.ERR] ' + data.message
                });
                break;
            case 'state':
    ...
    _transitionError: function (err) {
        if (err.code == 'BADSTATE') {
            this.logger.error('[BADSTATE] exp=%s cur=%s valid=%j', err.expected, err.state, err.accepts);
        } else {
            this.logger.logError(err, {
                message: '[TRANSIT.ERR] ' + err.message
            });
        }
    },
