...
var Network = Class(ManagementObject, {
    constructor: function (id, props, options) {
        ...
        this._serviceDefs = props['services'];
        Array.isArray(this._serviceDefs) || (this._serviceDefs = []);
    },
    ...
    dump: function () {
        return {
            ...
            adapter: this._adapterConf,
            services: this._serviceDefs
        };
    },
    ...
    destruct: function (callback) {
        flow.steps()
            .aggregateErrors()
            .next(flow.each(this._services).do('&stop'))
        ...
        flow.steps().chain()
            .next(function (next) {
                ...
                plugins.connect(this, 'cells:network.adapter', {
                    ...
                    data: this.pluginData(this._adapterConf, 'adapter'),
                    required: true
                }, next);
            })
    ...
    'setup:services': function (done) {
        this._services = {};
        flow.each(this._serviceDefs)
            .do(function (def, next) {
                if (def.id && def.role) {
                    plugins.connect(this, 'cells:network.service.' + def.role, {
                        data: this.pluginData(def, def.role)
                    }, function (err, svc) {
                        if (!err && svc) {
                            var monitor = this._services[def.id] = new ServiceMonitor(svc);
                            // subscribe events
                            monitor.start(next);
                        } else {
                            next();
                    ...
                    if (svc) {
                        this._services[def.id] = new ServiceMonitor(svc);
                    }
                } else {
                    next();
                }
...
module.exports = {
    ...
    create: function (id, props, refObjs, options, callback) {
        var network = new Network(id, props, options);
