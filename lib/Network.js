...
var Network = Class(ManagementObject, {
    ...
    setup: function (callback) {
        this.logger.debug('LOAD-PROVIDER');
        this.connectPlugin(
    ...
    dump: function () {
        ...
        return {
            ...
            provider: typeof(this.provider.dump) == 'function' ? this.provider.dump() : undefined
        };
    },
    ...
    destruct: function (callback) {
        flow.steps()
            .ignoreErrors()
            .next(this.provider ? this.provider.destroy.bind(this.provider) : function (next) { next(); })
            .next(ManagementObject.prototype.destruct)
            .with(this)
            .run(callback);
    }
});
